...
Prototype for dag-bandit-experiment. A zero-budget validation of confidence-gated tool-calling in a 2.3B LLM, replicating the DAG-Bandit framework
```markdown
...
## 🏗️ Architecture: The 3‑Node Verify DAG
...
2. **Upload the notebook** to [Google Colab](https://colab.research.google.com) (free T4 GPU works).
3. **Install dependencies** via `requirements.txt` (or just run the first cell of the notebook).
...
5. **Expected runtime:** ~5 minutes on a free Colab GPU.
...
- **Port to LangGraph** – full stateful orchestration with multiple DAG templates (Flat, Refine, Debate).
